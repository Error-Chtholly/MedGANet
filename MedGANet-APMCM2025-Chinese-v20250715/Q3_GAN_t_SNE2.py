...
# 绘制t-SNE图
def plot_tsne(original_data, generated_data, original_labels=None, title="t-SNE Visualization"):
    # 只保留原始正类和生成样本（过滤掉原始负类）
    ...
    color_labels = np.concatenate([
        np.ones(len(original_data)),  # 原始正类标记为1
        np.ones(len(generated_data)) * 2  # 生成样本标记为2
    ])
    ...
    tsne_df['Type'] = ['Original'] * len(original_data) + ['Generated'] * len(generated_data)

    # 设置颜色和样式（只保留红色和绿色）
    palette = {1: 'red', 2: 'green'}  # 1=原始正类, 2=生成样本
    markers = {'Original': 'o', 'Generated': 's'}
    ...
    legend_labels = {
        1: 'Original Positive',
        2: 'Generated Positive'
    }

    # 创建自定义图例
    legend_elements = [
        plt.Line2D([], [], marker='o', color='w', label=legend_labels[1],
                   markerfacecolor=palette[1], markersize=10),
        plt.Line2D([], [], marker='s', color='w', label=legend_labels[2],
                   markerfacecolor=palette[2], markersize=10)
    ]
...
def train_and_evaluate_with_gan(X, y, n_splits=10, epochs=100, batch_size=32):
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(X, y)):
        ...
        # 绘制t-SNE图比较原始样本和生成样本
        print("绘制t-SNE图比较原始样本和生成样本...")
        plot_tsne(
            minority_data,
            generated_samples,
            original_labels=y_train[y_train == 1],
            title=f"Fold {fold + 1}: t-SNE Visualization of Original vs Generated Samples"
        )
