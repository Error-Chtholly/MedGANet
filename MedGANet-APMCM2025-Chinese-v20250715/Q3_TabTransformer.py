...
class AttentionHead(nn.Module):
    ...
    def forward(self, x):
        ...
        dim_k = K.size(-1)
        scores = torch.bmm(Q, K.transpose(1, 2)) / torch.sqrt(torch.tensor(dim_k))
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        # x shape: (batch_size, seq_len, dim_in)
...
class TransformerEncoderLayer(nn.Module):
    def __init__(self, dim_in, num_heads, dim_feedforward, dropout=0.1):
        super().__init__()
        self.attention = MultiHeadAttention(num_heads, dim_in, dim_in, dim_in)
        self.norm1 = nn.LayerNorm(dim_in)
        self.dropout1 = nn.Dropout(dropout)

        self.linear1 = nn.Linear(dim_in, dim_feedforward)
        self.linear2 = nn.Linear(dim_feedforward, dim_in)
        self.telu = TeLU(alpha=0.15)
    ...
    def forward(self, x):
        ...
        # 前馈网络
        ff_out = self.linear2(self.telu(self.linear1(x)))
...
class TabTransformer(nn.Module):
    def __init__(self, input_size, num_heads=4, dim_feedforward=32, num_layers=3, dropout=0.1):
        super().__init__()
        # 假设每个特征都是独立的"词"，所以seq_len = input_size, dim_in = 1
        self.embedding = nn.Linear(1, 32)  # 将每个特征值嵌入到32维空间

        # Transformer编码器
        self.encoder_layers = nn.ModuleList([
            TransformerEncoderLayer(32, num_heads, dim_feedforward, dropout)
    ...
    def forward(self, x):
        ...
        # 分类
        x = self.flatten(x)  # (batch_size, input_size * 32)
        x = self.fc1(x)
        x = self.bn1(x)
        x = self.telu1(x)
        x = self.dropout1(x)
...
def train_and_evaluate_with_gan(X, y, n_splits=10, epochs=100, batch_size=32):
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(X, y)):
        ...
        # 初始化分类器模型
        model = TabTransformer(input_size=input_size)
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 50)
    print("使用GAN+TabTransformer+FocalLoss（带SMOTE过采样）进行模型训练...")
    print("=" * 50)
