...
def train_test_split(X, y, splits=10, batch_size=32):
    ...
    for fold, (train_idx, test_idx) in enumerate(k_fold.split(X, y)):
        ...
        if current_score > best_model_info['val_score']:
            ...
            best_model_info['model'] = model  # LightGBM模型可以直接保存
            best_model_info['fold'] = fold + 1
            best_model_info['scaler'] = scaler

        # 绘制并保存当前折的PR曲线
        precision, recall = plot_pr_curve(y_test, y_pred, fold + 1)
    ...
    # 保存最佳模型
    best_model_info['model'].save_model('cirrhosis_lgbm.txt')
    joblib.dump(best_model_info['scaler'], 'scaler.pkl')

    print(
        f"\nSaved best model from fold {best_model_info['fold']} with AUROC {best_model_info['val_score']:.4f} as cirrhosis_lgbm.txt")
