...
def print_coefficients(model, feature_names):
    """打印模型系数"""
    print("\n模型系数(权重):")
    print(f"{'特征':<15}{'Class 0':>10}{'Class 1':>10}{'Class 2':>10}{'Class 3':>10}")
    for i, name in enumerate(feature_names):
        print(f"{name:<15}", end="")
        for j in range(4):  # 4个类别
            print(f"{model.coef_[j][i]:>10.4f}", end="")
        print()
    print("\n截距项:")
    print(f"{'Class 0':>10}{'Class 1':>10}{'Class 2':>10}{'Class 3':>10}")
    print(
        f"{model.intercept_[0]:>10.4f}{model.intercept_[1]:>10.4f}{model.intercept_[2]:>10.4f}{model.intercept_[3]:>10.4f}")
...
def train_test_split(X, y, splits=10):
    ...
    feature_names = ['N_Days', 'Age', 'Bilirubin', 'Albumin', 'Copper',
    ...
    interp_precisions = [[] for _ in range(4)]
    ...
    }, 'cirrhosis_logreg.pkl')

    print(
        f"\nSaved best model from fold {best_model_info['fold']} with AUROC {best_model_info['val_score']:.4f} as cirrhosis_logreg.pkl")

    # 打印最终最佳模型的系数
    print("\n最佳模型系数:")
    print_coefficients(best_model_info['model'], feature_names)
